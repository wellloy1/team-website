...
`,
  ...
  properties: {
     ...
     */
    opened:
        {type: Boolean, value: false, notify: true, reflectToAttribute: true, observer: '_openedChanged'},
	    
    trackHistory: { type: Boolean, value: false, notify: true, reflectToAttribute: true },
  ...
  attached: function() {
    afterNextRender(this, function() {
      ...
      this.setScrollDirection('y', this.$.contentContainer);

      window.addEventListener("popstate", this._onHistoryPopstate.bind(this))
      if (window.history.state && window.history.state['app-drawer'])
      {
        const hndl = () =>
        {
        ...
        if (Debouncer) 
        {
          this._openDebounce = Debouncer.debounce(this._openDebounce, microTask, hndl)
        }
        else
        {
          hndl()
  ...
  close: function() {
    var old = this.opened
    this.opened = false;
    if (window.history.length > 0 && !this._suppressHistory && old && this.trackHistory)
    {
      window.history.back()
    }
  },
  ...
  _calculateVelocity: function(event, trackDetails) {
    ...
    if (trackDetail && event && event.detail) {
  ...
  _onHistoryPopstate: function (e)
  {
    ...
    if (this._lastState && this._lastState['app-drawer'])
    {
      this._lastState = e['state']
      this.close()
    }
    else if (e && e.state && e.state['app-drawer'])
    {
      this._lastState = e.state
      this.open()
    }
  ...
  {
    // console.log('history: ' + window.history.length, this._suppressHistory, opened, old)

    if (!opened && old == undefined) { return }

    if (opened)
    {
      if (!this._suppressHistory && this.trackHistory)
      {
        var state = { 'app-drawer': true }
        var title = 'App-Drawer'
        var url = window.location.href.replace(window.location.hash, "") +  '#app-drawer'
