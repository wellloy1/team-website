...
class RichTextEditorElement extends ElementMixin(ThemableMixin(PolymerElement)) {
  static get template() {
    ...
    <style include="vaadin-rich-text-editor-styles">
      ...
      paper-swatch-picker.ql-color {
        display: block;
        width: unset;
        height: unset;
        --paper-swatch-picker-menu-btn : {
          padding: 0px;
        }
      }
      ...
      <div part="toolbar">
        ...
        <span part="toolbar-group toolbar-group-emphasis">
          ...
          <paper-swatch-picker 
            ...
            icon="admin-editor:format-color-text" 
            no-animations
            color="{{selectedColor}}">
          </paper-swatch-picker>
    ...
    <vaadin-confirm-dialog id="imgUrlDialog" opened="{{_imgUrlEditing}}" header="[[i18n.imageUrlEmbeddedMsg]]">
      <vaadin-text-field id="imgUrl" value="{{_imgUrl}}" style="width: 100%;" on-keydown="_onImgUrlKeydown"></vaadin-text-field>
      <vaadin-button id="confirmImgUrl" slot="confirm-button" theme="primary" on-click="_onImgUrlEditConfirm">
        [[i18n.ok]]
      </vaadin-button>
      <vaadin-button id="cancelImgUrl" slot="cancel-button" on-click="_onImgUrlEditCancel">
        [[i18n.cancel]]
      </vaadin-button>
...
`;
  ...
  static get properties() {
    return {
      ...
      i18n: {
        ...
        value: () => {
          return {
            ...
            cancel: 'Cancel',
            remove: 'Remove',
            imageUrlEmbeddedMsg: 'Insert URL as Image',
          };
        }
      },
      ...
      selectedColor: { type: String, value: '', notify: true, observer: '_selectedColorChanged'},
  ...
  ready() {
    ...
    this.$.imgUrlDialog.$.dialog.$.overlay.addEventListener('vaadin-overlay-open', () => {
      this.$.imgUrl.focus();
    });
  ...
  /** @private */
  _onImageClick() {
    this._imgUrlEditing = true;
    // this.$.fileInput.value = '';
    // this.$.fileInput.click();
  }

  _onImgUrlEditConfirm()
  {
    const image = this._imgUrl;
    const range = this._editor.getSelection(true);
    this._editor.insertEmbed(range.index, 'image', image, Quill.sources.USER);
    this._markToolbarClicked();
    this._editor.setSelection(range.index + 1, SOURCE.SILENT);
    this._closeImgUrlDialog();
  }
  ...
  _onImgUrlKeydown(e)
  {
    if (e.keyCode === 13)
    {
      e.preventDefault();
      e.stopPropagation();
      this.$.confirmImgUrl.click();
    }
  }

  _selectedColorChanged(col, old)
  {
    if (!col) { return }
    this._editor.format('color', col);
  }
