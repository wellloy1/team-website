...
import view from './product-labels.ts.html'
import style from './product-labels.ts.css'
...
@FragmentDynamic
export class AdminProductLabels extends FragmentBase
{
    static get is() { return 'tmladmin-product-labels' }
    ...
    static get properties()
    {
        return {
            ...
            APIPath: { type: String, value: '/admin/api/productlabel/' },
    ...
    newProductLabelTap(e)
    {
        var url = this._urlViewProductLabel('_new_')
        this._goto(url)
    ...
    formatZPLBody(itemZPLBody)
    {
        if (!itemZPLBody) return itemZPLBody

        let len = 180
        let str = itemZPLBody.substr(0, len)
        if (itemZPLBody.length > len)
        {
            str += "..."
        }
        // if (itemZPLBody.length > len * 2)
        // {
        //     str += itemZPLBody.substr(itemZPLBody.length - len, len)
        // }
        return str
    }

    _imageLoadedDebouncer: Debouncer
    _onPreviewLoaded(e?)
    {
        // console.log(e)
        var img = e.target
        this._imageLoadedDebouncer = Debouncer.debounce(this._imageLoadedDebouncer, timeOut.after(190), () =>
		{
            if (img) 
            { 
                img.dispatchEvent(new CustomEvent('iron-resize', { bubbles: true, composed: true, detail: { } }))
            }
		})
    }
