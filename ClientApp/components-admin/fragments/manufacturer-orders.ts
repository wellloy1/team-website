...
import view from './manufacturer-orders.ts.html'
import style from './manufacturer-orders.ts.css'
...
@FragmentDynamic
export class AdminManufactureOrders extends FragmentBase
{
    static get is() { return 'tmladmin-manufacturer-orders' }
    ...
    static get properties()
    {
        return {
            ...
            APIPath: { type: String, value: '/admin/api/manufacture/order-' },
            ...
            showenBuildReportBtn: { type: Boolean, computed: '_compute_showenBuildReportBtn(userInfo.isAdmin, userInfo.isPrinter)' },
    ...
    _batchesList(itemSets)
    {
        if (!itemSets) { return '' }
        return itemSets.map(i => i.ManufactureItemSetID)
    }
    ...
    _onDownload(e) 
    {
        ...
        this.getOrderFile({ oid: oid, id: id }, progress)

        e.preventDefault()
        return false
    }

    _compute_showenBuildReportBtn(isAdmin, isPrinter)
    {
