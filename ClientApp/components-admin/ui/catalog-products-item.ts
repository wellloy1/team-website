...
import view from './catalog-products-item.ts.html'
import style from './catalog-products-item.ts.css'

const Teamatical: TeamaticalGlobals = window['Teamatical']


@CustomElement
export class AdminCatalogProductsItem extends UIBase
{
    static get is() { return 'tmladmin-catalog-products-item' }
    ...
    ready()
    {
        ...
        this.addEventListener('tap', (e) =>
        {
            ...
            {
                var epath: any = (e as any).path || e.composedPath()
                var txt = epath[0]
    ...
    newProductTap(e)
    {
        ...
        this.dispatchEvent(new CustomEvent('api-admin-catalog-item-new', {
    ...
    _deleteTap(e)
    {
        ...
        for (var i in epath)
        {
            ...
            if (epath[i] instanceof AdminCatalogProductsItem) 
            {
                listitem = epath[i]
                if (listitem && listitem.__data)
                {
                    ...
                    this.async(() =>
                    {
                        this.dispatchEvent(new CustomEvent('api-admin-catalog-item-delete', { bubbles: true, composed: true, detail: { item: data } }))
                        this.deleting = false
                    }, 200)
                }
                break
