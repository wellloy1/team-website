...
module.exports = merge(baseConfig(config), {
	...
	output: {
		...
		path: resolve(__dirname, '../..', config.outputDir, config.app.version),
		publicPath: 'build/' + config.app.version + '/',
	},
	...
	plugins: [
		new CleanWebpackPlugin({ 
			verbose: true, 
			cleanOnceBeforeBuildPatterns: [resolve(__dirname, '../..', config.outputDir)],
		...
		new InjectManifestPlugin({
			...
			swDest: resolve(__dirname, '../..', config.outputDir, 'service-worker.js'),
			exclude: [
				/\.map$/,
				/manifest$/,
				/service-worker\.js$/,
