...
import '../bll/net-base'
import view from './ui-assets-uploader.ts.html'
import style from './ui-assets-uploader.ts.css'
...
@CustomElement
export class UIAssetsUploader extends UIImageUploaderBase
{
    static get is() { return 'teamatical-ui-assets-uploader' }
    ...
    static get properties()
    {
        return {
            ...
            apiPath: { type: String, reflectToAttribute: true },
            visible: { type: Boolean },

            assets: { type: Array, notify: true, value: () => { return [] } },
            ...
            files: { type: Array, notify: true, readOnly: true },
    ...
    _log(v) { console.log('ui-assets-uploader', v) }
        ...
        if (blob !== null)
        {
            ...
            reader.onload = (ef) =>
            {
                ...
                var files = [
                    {
                        type: item.type,
                        name: this.localize('assetsuploader-filename-fromclipboard'),
    ...
    _onFileDrop(e)
    {
        ...
        {
            let message = this.localize('assetsuploader-multifiles-error', 'files_count', files.length)
    ...
    _validate(fileList)
    {
        ...
        for (let i = 0, len = fileList.length; i < len; ++i)
        {
            ...
            let message = this.localize('assetsuploader-accepttype-error', 'files_name', fileList[i].name)
            this._setLastError(new ErrorEvent('error', { message }))
            return false
        }
    ...
    _filesHasChanged(files)
    {
        ...
        for (var i in files)
        {
            ...
            var imgURL = fi.imgData ? '' : DOMURL.createObjectURL(fi)
    ...
    async _uploadFile(file, inx)
    {
        ...
        var rq = {
            url: this.websiteUrl + '/api/v1.0/' + this.apiPath,
            body: formData,
            method: "POST",
            handleAs: "blob",
            contentType: null,
    ...
    _onUploadProgress(e, file, inx)
    {
        if (e.lengthComputable)
        {
            ...
            if (p >= 100)
            {
                this.set('files.' + inx + '.progressText', this.localize('assetsuploader-processing'))
    ...
    _onUploadResponse(e, file, inx)
    {
        ...
        if (r && r['success'])
        {
            ...
            //merge
            var ids = Object.assign({}, ...this.assets.map(({ AssetID, FileName }) => ({ [AssetID]: { AssetID, FileName  } })))
            ids[obj.AssetID] = obj
            var numeric_array = []
            for (var i in ids) { numeric_array.push(ids[i]) }
            this.set('assets', numeric_array)
            // console.log(obj, numeric_array)

            this.set('files.' + inx + '.progressText', this.localize('assetsuploader-done'))
    ...
    {
        this.set('files.' + inx + '.done', true)
        this.set('files.' + inx + '.progressText', this.localize('assetsuploader-error'))
        console.error(e)
    }
}