...
import { GroupShippingListModel } from '../dal/groupshipping-list-model'
import { GroupShippingListItemModel } from '../dal/groupshipping-list-item-model'
import { CustomElement } from '../utils/CommonUtils'
const ptoken_empty = ""


@CustomElement
export class StoreGroupsData extends NetBase
{
    static get is() { return 'teamatical-store-groups-data' }
    ...
    _computeAPIUrl(api_action, websiteUrl)
    {
        ...
        var url = this.websiteUrl + '/api/v1.0/store/groupshipping-list-' + api_action
        return url
    }
    ...
    _fetchItems(attempts)
    {
        ...
        var obj = null
        if (this.api_action == 'get' && !(this.stores && this.stores.ptoken) && !(this.stores && this.stores.search))
        {
        ...
        if (!r || r['success'] !== true)
        {
            ...
            if (summary && summary.Key == 'not_logged')
            {
                ...
                this.dispatchEvent(new CustomEvent('api-show-dialog', {
                    bubbles: true, composed: true, detail: {
                        required: true,
                        announce: this.localize('store-groupshipping-list-announce'),
    ...
    {
        if (!res) { return new GroupShippingListModel() }

        var slist = Object.assign((update ? update : new GroupShippingListModel()), res)
    ...
    {
        return Object.assign((update ? update : new GroupShippingListItemModel()), res)
    }
