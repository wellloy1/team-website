...
import { OrderRosterListModel, OrderRosterItemModel } from '../dal/order-roster-list-model'
import { CustomElement } from '../utils/CommonUtils'
const ptoken_empty = ""


@CustomElement
export class OrderRosterData extends NetBase
{
    static get is() { return 'teamatical-order-roster-data' }
    ...
    reload(reset?)
    {
        ...
        this.api_action = 'list-get'
        if (this.queryParams && this.queryParams.oid) 
        { 
            if (this.orders) 
            {
                this.setProperties({
                    'orders.id': this.rid,
                    'orders.oid': this.queryParams.oid,
                })
            }
            else
            {
    ...
    _computeAPIUrl(api_action, websiteUrl)
    {
        ...
        var apiPath = '/api/v1.0/user/order-rosteritems-'
    ...
    _fetchItems(attempts, oid?)
    {
        ...
        // if (qp && qp['accessToken']) 
        // { 
        //     this._setAccessToken(qp['accessToken'])   //--disabled due lineitemid not secure for now
    ...
    {
        if (!res) { return new OrderRosterListModel() }

        var slist = Object.assign((update ? update : new OrderRosterListModel()), res)
    ...
    {
        return Object.assign((update ? update : new OrderRosterItemModel()), res)
    }
