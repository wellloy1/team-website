...
export class Color
{
	static css_hex2rbg = (hex, o:number | null = null) =>
	{
		if (!hex || typeof (hex) !== 'string') { return hex }
		
		var {r, g, b} = Color.hex2rgbobj(hex)

		return Number.isFinite(o) ? `rgba(${r},${g},${b},${o})` : `rgb(${r},${g},${b})`
	}
	...
	static invertColor(hex, bw?)
	{
		...
		return "#" + r.padStart('00') + g.padStart('00') + b.padStart('00')
	}

	static nearestColor(needleStr, colorsPalette)
	{
		// '#3D3835'
		var needle
		try
		{ 
			needle = parseColor(needleStr)
		}
		catch
		{
			//
		...
		var distanceSq, minDistanceSq = Infinity, rgbi, value

		for (var i = 0; i < colorsPalette.length; ++i)
		{
			if (!colorsPalette[i].i) { continue } //skip separotors

			if (!colorsPalette[i].rgb)
			{
				// console.log(colorsPalette[i])
				try
				{
					colorsPalette[i].rgb = parseColor(colorsPalette[i].h)
				}
				catch
				{
					//
			...
			rgbi = colorsPalette[i].rgb

			distanceSq = colorDiffRGB(needle, rgbi)
			if (distanceSq < minDistanceSq)
			{
				minDistanceSq = distanceSq
				value = colorsPalette[i]
			}
		}

		// console.log(needle, value, Math.sqrt(minDistanceSq))
	...
	static contrastBW(hexcolor:string)
	{
...
//#region Nearest Color Utils

function colorDiffRGB(c1, c2)
{
	return Math.sqrt(
		(c1.r - c2.r) * (c1.r - c2.r)
		+ (c1.g - c2.g) * (c1.g - c2.g)
...
// {
// 	return (rgb.r * 0.299 + rgb.g * 0.587 + rgb.b * 0.114) / 256.0
// }


// function getHueDistance(hue1, hue2)
// {
