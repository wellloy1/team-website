...
import view from './home-fashiontv.ts.html'
import style from './home-fashiontv.ts.css'
...
@FragmentDynamic
export class HomeFashionTV extends HomeBase
{
    static get is() { return 'teamatical-home-fashiontv' }
    ...
    static get properties()
    {
        return {
            ...
            recentSlide: { type: Number, value: 0, reflectToAttribute: true  },
            slides: { type: Array, value: [] },
            resizeWidth: { type: Number, value: 6000  },
            framei: { type: Number, value: 1  },
    ...
    connectedCallback()
    {
        ...
        if (div_list && container)
        {
            ...
            this.frameTimer = setInterval((e) => this.frameTimerHandler(e), 2000)
        }


        this.addEventListener('teamatical-ui-image-done', (e) => this._onSlideLoaded(e))
        

        var link = document.createElement("link")
        link.rel = 'stylesheet'
        link.type = 'text/css'
        link.href = '/images/home-fashiontv/font/stylesheet.css'
    ...
    frameTimerHandler(e)
    { 
        var framei = this.framei
        framei++ 
        if (framei > 3) { framei = 1 }
        if (this.lastFrame < this.loadedFrame)
        {
            this.framei = framei
            this.lastFrame = this.loadedFrame
        }
    }

    _onSlideLoaded(e)
    {
        if (e?.detail?.img?.parentElement?.style?.visibility == 'visible')
        {
            this.loadedFrame++
        }
    }
